...
# class AdamW(Adam):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                # grad = p.opt_grad(self)
                if p.opt_grad(self) is None:
                    continue
                grad = p.opt_grad(self).data
                # if jt.sparse(grad):
                #     raise RuntimeError("Adam does not support sparse gradients, please consider SparseAdam instead")

                # state = self.state_dict()['defaults']['param_groups'][p]
                state = self.state[p]
...
def create_adam_optimizer_from_args(model, args, grouped_parameters=None):
    ...
    # )
    # 创建一个Adam optimizer
    optimizer = jt.optim.Adam(model.parameters(), lr=args.lr, eps=args.adam_epislon, 
