...
# from jittor.amp import scale_loss
from jittor.dataset import DataLoader
# from torch.utils.data import DataLoader
# torch.set_printoptions(threshold=100000)

import torch
import model_torch    # 载入预训练的checkpoint需要用到torch，将torch模型转换为jittor模型

# # 设置全局日志级别
# jt.set_global_log_level(3)  # 设置为 3 表示打印详细信息
...
from model_jittor import GPT2Config, GPT2LMModel
...
parser.add_argument('--init_checkpoint', default='./pretrained_checkpoints/gpt2-pytorch_model.bin', help='pretrained checkpoint path')
...
parser.add_argument('--work_dir', type=str, default='./trained_models/GPT2_M2/e2e', 
                    help='working folder.')
...
def optimizer_step(_loss, _optimizer, _model, _schedule, args, is_update=True):
    ...
    if is_update:
        # if args.clip > 0:
        #     if args.fp16:
        #         # nn.utils.clip_grad_norm_(amp.master_params(_optimizer), args.clip)
        #         _optimizer.clip_grad_norm(amp.master_params(_optimizer), args.clip)
        #     else:
        #         # nn.utils.clip_grad_norm_(_model.parameters(), args.clip)
        #         _optimizer.clip_grad_norm(amp.master_params(_optimizer), args.clip)
...
if __name__ == '__main__':
    ...
    #         print('Could not import amp, apex may not be installed')
    #         # warnings.warn('Could not import amp, apex may not be installed')

    # torch.manual_seed(args.random_seed)

    # random.seed(args.random_seed)
    # jt.flags.seed = args.random_seed  # flags没有seed
    # 设置 Jittor 的随机种子
    ...
    if args.model_card == 'gpt2.sm':
        config = model_torch.GPT2Config_torch(
            n_embd=768, n_layer=12, n_head=12, 
            lora_attn_dim=args.lora_dim, 
            lora_attn_alpha=args.lora_alpha, 
            lora_dropout=args.lora_dropout,
        )
        config_jittor = GPT2Config(
            n_embd=768, n_layer=12, n_head=12, 
            lora_attn_dim=args.lora_dim, 
            lora_attn_alpha=args.lora_alpha, 
            lora_dropout=args.lora_dropout,
    ...
    # lm_net_torch = GPT2LMModel_torch(config_jit)
    # lm_net = GPT2LMModel(config_jittor)
    # if args.init_checkpoint is not None:
    #     print('loading model pretrained weight.')
    #     lm_net.load_weight(jt.load(args.init_checkpoint))    

    # lm_net = lm_net.cuda()
    lm_net_torch = model_torch.GPT2LMModel_torch(config)
    if args.init_checkpoint is not None:
        print('loading model pretrained weight.')
        lm_net_torch.load_weight(torch.load(args.init_checkpoint))    

    lm_net_torch = lm_net_torch.cuda()
    print("torch model: ")
    print(lm_net_torch)
    # print(lm_net_torch.state_dict())
    # 如果有lm_net.pth就删除它
    if os.path.exists('/root/HW5/LoRA/examples/NLG/src_jittor/t2j/lm_net.pth'):
        os.remove('/root/HW5/LoRA/examples/NLG/src_jittor/t2j/lm_net.pth')
    torch.save(lm_net_torch.state_dict(), '/root/HW5/LoRA/examples/NLG/src_jittor/t2j/lm_net.pth')
    # 将lm_net_torch占用的内存释放
    del lm_net_torch
    lm_net = GPT2LMModel(config_jittor)
    lm_net.load('/root/HW5/LoRA/examples/NLG/src_jittor/t2j/lm_net.pth')
    print("jittor model: ")
    print(lm_net)
