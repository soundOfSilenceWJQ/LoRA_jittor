...
class LMOrderedIterator(object):
    ...
    def get_fixlen_iter(self, start=0):
        self.data_len = self.split_data.shape[1]
        _eval_cursor = 0
...
class BinLMOrderedIterator(object):
    def __init__(self, corpus, bsz, bptt, eval_len=None, device='cpu', world_size=1, rank=0):
        ...
        self.device = device
        self.global_bsz = bsz * world_size
        self.n_step = corpus.length // self.global_bsz
    ...
    def get_batch(self, i, bptt, eval_len):
        ...
        _input = jt.array(_inputs, dtype='int64').contiguous()
        _target = jt.array(_targets, dtype='int64').contiguous()
...
class FT_Dataset(Dataset):
    def __init__(self, ft_file, batch_size, max_seq_length, 
                 ...
                 prefix_cursor=1000000, infix_cursor=2000000, shuffle=True, num_workers=0, pin_memory=False, drop_last=False):
    ...
    def __getitem__(self, item):
        ...
        output = {}
        output["id"] = jt.array(item, dtype='int64')
        ...
        output["query"] = jt.array(_query, dtype='int64')
        output["query_len"] = jt.array(_query_len, dtype='int64')

        output["input"] = jt.array(_input, dtype='int64') 
        output["target"] = jt.array(_target, dtype='int64') 

        output["mask"] = jt.array(_msk, dtype='float32')
        return output
