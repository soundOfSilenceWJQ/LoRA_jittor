...
class LayerNorm(jt.Module):
    def __init__(self, hidden_size, eps=1e-12):
        """Construct a layernorm module in the TF style (epsilon inside the square root)."""
        super(LayerNorm, self).__init__()
        self.weight = jt.nn.Parameter(jt.ones([hidden_size]))
        self.bias = jt.nn.Parameter(jt.zeros([hidden_size]))
        self.variance_epsilon = eps
        # self.weight = jt.init.ones([hidden_size])
        # self.bias = jt.init.zeros([hidden_size])
        # self.variance_epsilon = eps
    ...
    def execute(self, x, history=None, layer_past=None, len_past=None):
        ...
        if layer_past is not None:
            ...
            if len_past is None:
                ...
                _batch = jt.arange(0, key.shape[0], dtype=jt.int32)
