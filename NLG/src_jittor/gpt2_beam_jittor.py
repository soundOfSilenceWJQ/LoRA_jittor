...
def _reorder_cache(past: Tuple, beam_idx: jt.Var) -> Tuple[jt.Var]:
    return tuple(jt.gather(layer_past, dim=1, index=beam_idx.expand_as(layer_past, dims=[0, 2, 3, 4])).detach() for layer_past in past)
    # return tuple(torch.index_select(layer_past, dim=1, index=beam_idx).detach() for layer_past in past)
...
def _calc_banned_ngram_tokens(
    ...
    for idx in range(num_hypos):
        gen_tokens = prev_input_ids[idx].numpy()
...
def _enforce_repetition_penalty_(
    ...
    for i in range(batch_size * num_beams):
        ...
        for previous_token in set(prev_output_tokens[i].numpy()):
...
def beam(model, data_iter, args):
    ...
    with jt.no_grad():
        for idx, data in enumerate(data_iter):
            ...
            _batch = jt.arange(0, _id.size(0), dtype=jt.int32)
            # _batch = jt.arange(0, _id.size(0), device=args.device, dtype=jt.long)
            past = None
            len_past = None
            ...
            #     (batch_size, num_beams), dtype=jt.float, device=_query.device
            # )
            beam_scores = jt.zeros(
                (batch_size, num_beams), dtype=jt.float
            )
            # best_sequence = jt.zeros(
            #     (batch_size, args.eval_len), dtype=jt.long, device=_query.device
            # )
            best_sequence = jt.zeros(
                (batch_size, args.eval_len), dtype=jt.int32
            )
            best_score = {}
            ...
            with jt.no_grad():
                for i in range(0, args.eval_len):
                    ...
                    next_scores, next_tokens = jt.misc.topk(
            ...
            if args.rank == 0:
                ...
                for _b in range(0, _id.shape[-1]):
                    ...
                    all_predictions[_i]['predict'] = output[_b]
...
if __name__ == '__main__':
    ...
    # 取valid_loader的前30个batch
    # valid_loader = itertools.islice(valid_loader, 1)

    if args.model_card == 'gpt2.sm':
        config = GPT2Config_jittor(
    ...
    lm_net = GPT2LMModel_jittor(config)
